...
# ✅ Устанавливаем переменную окружения, чтобы Django не пытался писать в /app/logs
export DISABLE_CELERY_LOGFILE="1"

echo "✅ Dependencies are ready. Applying migrations..."
"$PYTHON_PATH" manage.py migrate --noinput

echo "🚀 Starting Celery beat..."
exec "$PYTHON_PATH" -m celery -A DiplomNetologyGjango beat -l INFO --scheduler django_celery_beat.schedulers:DatabaseScheduler